...
def graph(tLev, tDL, tDLR, tDLRC, length):
    plt.figure(1)
    plt.plot(length, tLev, label = "Левенштейн")
    plt.plot(length, tDL, label = "Дамерау-Левенштейн (матричный)")
    ...
    plt.figure(2)
    plt.plot(length, tDL, label = "Дамерау-Левенштейн (матричный)")
    plt.plot(length, tDLR, label="Дамерау-Левенштейн (рекурсия)")
    plt.plot(length, tDLRC, label="Дамерау-Левенштейн (с кэшем)")
    plt.legend(fontsize=12)
    plt.xlabel('Длина строки')
    plt.ylabel('Время, мс')
    plt.grid()
